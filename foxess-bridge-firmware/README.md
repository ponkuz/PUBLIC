...
`lilygo_t_can485_sungrow_active_test` is the controlled active test package. It answers the Sungrow inverter as an emulated DTSU666 meter while keeping SD diagnostics enabled, including active Modbus request/response logs. Use it only after the shadow package has been validated on the same installation.
